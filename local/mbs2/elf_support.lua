...
local tLpeg_Support = require 'mbs2.lpeg_support'

-- Save typing:
local P, V, Cg, Ct, Cc, S, R, C, Cf, Cb, Cs, match, OptionalSpace,Space,Comma =
...
local function executeCmd(tCmd)
  ...
  local fResult,strReturnCode,strStdout,strError = utils.executeex(strCmd)
...
function Elf_Support:get_segment_table(strToolObjdump, strFileName, astrSegmentsToConsider)
  local Set = require 'pl.Set'
  local astrSegmentsToConsiderSet = (astrSegmentsToConsider ~= nil) and Set(astrSegmentsToConsider) or nil
  ...
  for strLine in stringx.lines(strStdout) do
...
--- Extract the entry point address from the ELF file header.
...
function Elf_Support:get_exec_address(strToolReadelf, strFileName)
...
function Elf_Support:get_symbol_table(strToolReadelf, strFileName)
...
function Elf_Support:get_macro_definitions(strToolReadelf, strFileName)
  -- Macro symbol table
  local atElfMacros = {}
...
-- @return atRootTree_DebugInfo tree structure of debug_info as table
  ...
  --
function Elf_Support:get_debug_structure(strToolReadelf, strFileName)
...
function Elf_Support:__iter_debug_info(atSymbols,atRootTree_DebugInfo,tSearchPattern,fExtractData)
  ...
  local function iterRootTree(tTree,uiSearchPatternLevel)
    ...
    for _,tNode in pairs(tTree) do
      ...
      if tablex.size(tNode.tChildNodes) > 0 then
...
function Elf_Support:get_debug_symbols(strToolReadelf, strFileName)

  local atElfDebugSymbols = {}

  -- get debug info tree node table
  local atRootTree_DebugInfo = self:get_debug_structure(strToolReadelf,strFileName)
